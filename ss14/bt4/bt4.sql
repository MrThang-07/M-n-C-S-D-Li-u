...
BEGIN
    ...
    DECLARE v_balance DECIMAL(18,2) DEFAULT 0.00;
    DECLARE v_wallet_status VARCHAR(20);
    
    -- Xử lý an toàn: Bắt lỗi hệ thống
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
        ROLLBACK;
        SET p_status_message = 'Lỗi hệ thống: Giao dịch thanh toán bị hủy.';
    END;

    -- KIỂM TRA ĐẦU VÀO TRƯỚC KHI MỞ TRANSACTION
    IF p_amount <= 0 THEN
    ...
    ELSE
        ...
        -- Khóa dòng dữ liệu ví của bệnh nhân để tránh tình trạng "Double Spending"
        SELECT balance, status INTO v_balance, v_wallet_status
        FROM Wallets 
        WHERE patient_id = p_patient_id 
        FOR UPDATE;

        -- KIỂM TRA NGHIỆP VỤ (Chiến lược 2)
        ...
        ELSEIF v_balance < p_amount THEN
            ...
            ROLLBACK;
        ELSE
            -- Thao tác 1: Trừ tiền trong Ví điện tử
            UPDATE Wallets 
            SET balance = balance - p_amount
            WHERE patient_id = p_patient_id;

            -- Thao tác 2: Giảm trừ công nợ
